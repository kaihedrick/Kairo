// filepath: BibleAppPOCV2/Tokenizer/BARTTokenizer.swift
import Foundation
...
class BARTTokenizer {
    ...
    /// Tokenize text directly to token IDs (Int32 version for LLM)
    func tokenizeToIds(_ text: String) -> [Int32] {
        print("🔤 Tokenizing text to IDs: \(text.prefix(50))...")
        ...
        var tokens: [Int32] = [Int32(vocabulary[bosToken] ?? 0)]
        ...
        for word in words {
            if let tokenId = vocabulary[word] {
                ...
                tokens.append(Int32(vocabulary[unkToken] ?? 3))
            }
        }
        
        tokens.append(Int32(vocabulary[eosToken] ?? 2))
        
        print("🔢 Tokenized to \(tokens.count) tokens")
        return tokens
    }
    
    /// Tokenize text into string tokens (renamed to avoid ambiguity)
    func tokenizeToStrings(_ text: String) -> [String] {
    ...
    func encode(_ text: String, maxLength: Int = 256) -> [Int] {
        let tokens = tokenizeToStrings(text)
...
extension BARTTokenizer {
    ...
    func batchTokenize(_ texts: [String]) -> [[String]] {
        return texts.map { tokenizeToStrings($0) }
    }
